...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        mbDate: { type: 'string', description: 'Transaction date (YYYY-MM-DD)' },
        ...
        inOutCode: { type: 'string', enum: ['0', '1'], description: '0=Income, 1=Expense' },
        ...
        mbDetailContent: { type: 'string', description: 'Optional: Detailed notes' },
      },
      required: ['mbDate', 'assetId', 'payType', 'mcid', 'mbCategory', 'mbCash', 'inOutCode', 'inOutType'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      required: ['id', 'mbDate', 'assetId', 'payType', 'mcid', 'mbCategory', 'mbCash', 'inOutCode', 'inOutType'],
    },
  },
  {
  ...
  },
  {
    ...
    description: 'Exports transaction data to Excel file. The server returns an HTML-based Excel format. Use .xls extension for best compatibility (if .xlsx is provided, it will be auto-corrected to .xls with a warning).',
    inputSchema: {
      type: 'object' as const,
      properties: {
        startDate: { type: 'string', description: 'Start date (YYYY-MM-DD)' },
        ...
        assetId: { type: 'string', description: 'Optional: Filter by asset ID' },
        inOutType: { type: 'string', description: 'Optional: Filter by transaction type' },
        outputPath: { type: 'string', description: 'Local path to save the Excel file (use .xls extension for best compatibility)' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        linkAssetId: { type: 'string', description: 'Optional: Linked asset ID' },
        linkAssetName: { type: 'string', description: 'Optional: Linked asset name' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      required: ['assetId', 'assetGroupId', 'assetGroupName', 'assetName', 'assetMoney'],
    },
  },
  {
    name: 'asset_delete',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        linkAssetName: { type: 'string', description: 'Linked payment asset name' },
        notPayMoney: { type: 'number', description: 'Unpaid balance (negative value)' },
        jungsanDay: { type: 'number', description: 'Optional: Balance calculation day (1-31)' },
        paymentDay: { type: 'number', description: 'Optional: Payment due day (1-31)' },
      },
  ...
  // Transfers
  {
    ...
    inputSchema: {
      ...
      required: ['moveDate', 'fromAssetId', 'fromAssetName', 'toAssetId', 'toAssetName', 'moveMoney'],
    },
  },
  {
    name: 'transfer_update',
    description: 'Modifies an existing transfer. WARNING: The server creates a new transfer with a NEW ID instead of updating in-place. The old ID will no longer exist after update. Use transaction_list to get the new ID if needed.',
    inputSchema: {
      type: 'object' as const,
      properties: {
      ...
      required: ['id', 'moveDate', 'fromAssetId', 'fromAssetName', 'toAssetId', 'toAssetName', 'moveMoney'],
    },
  },
  ...
  // Dashboard
  {
    ...
    description: 'Retrieves dashboard overview with asset trends and portfolio breakdown.',
    inputSchema: {
      type: 'object' as const,
      properties: {},
    },
...
class MoneyManagerMcpServer {
  ...
  async start(): Promise<void> {
    try {
      ...
      console.error('[MCP Server] Money Manager MCP Server started successfully.');
    } catch (error) {
